...
def initialize_u_torch(X, U0, V0, error=1e-4, maxIter=1000, minIter=30, meanFitRatio=0.1, initConv=1,
...
def pFN_SR_NMF_torch(Data, gFN, gNb, maxIter=1000, minIter=30, meanFitRatio=0.1, error=1e-4, normW=1,
    ...
    if isinstance(logFile, str):
        logFile = open(logFile, 'a')
    print(f'\nStart NMF for pFN using PyTorch at ' + time.strftime('%Y-%m-%d %H:%M:%S',
    ...
    for i in range(1, 1 + maxIter):
        ...
        L21 = alphaS * torch.sum(
            torch.sum(torch.sqrt(tmpl21), dim=0) / torch.maximum(torch.sqrt(torch.sum(tmpl21, dim=0)), torch_eps))
        LDf = tmp1
        LSl = torch.sum(tmp2)
...
def gFN_SR_NMF_torch(Data, K, gNb, init='random', maxIter=1000, minIter=200, error=1e-8, normW=1,
    ...
    if isinstance(logFile, str):
        logFile = open(logFile, 'a')
    print(f'\nStart NMF for gFN using PyTorch at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                   time.localtime(time.time())) + '\n', file=logFile,
          flush=True)
    ...
    for repeat in range(1, 1 + nRepeat):
        ...
        for i in range(1, 1 + maxIter):
            ...
            if alphaS > 0:
                tmpl21 = torch.sqrt(tmpl2)
                tmpl22 = torch.sqrt(torch.sum(tmpl2, dim=0,
                                              keepdim=True))  # tmpl22 = torch.tile(torch.sqrt(torch.sum(tmpl2, dim=0)), (dim_space, 1))
                tmpl21s = torch.sum(tmpl21, dim=0,
                                    keepdim=True)  # tmpl21s = torch.tile(torch.sum(tmpl21, dim=0), (dim_space, 1))
...
def gFN_fusion_NCut_torch(gFN_BS, K, NCut_MaxTrial=100, dataPrecision='double', logFile='Log_gFN_fusion_NCut'):
    ...
    if isinstance(logFile, str):
        logFile = open(logFile, 'a')
    print(f'\nStart NCut for gFN fusion using PyTorch at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                           time.localtime(time.time())) + '\n',
          file=logFile, flush=True)
    ...
    nW = torch.exp(-torch.pow(nDis, 2) / (torch.pow(torch.median(nDisVec),
                                                    2)))  # Transform distance values using exp(-X/std^2) with std as the median value
    nW[torch.isnan(nW)] = 0
    sumW = torch.sum(nW, dim=1)  # total distance for each FN
    sumW[sumW == 0] = 1  # In case two FNs are the same
    # Construct Laplacian matrix
    D = torch.diag(sumW)
    L = torch.sqrt(torch.linalg.inv(D)) @ nW @ torch.linalg.inv(
        torch.sqrt(D))  # A way to normalize nW based on the total distance of each FN
    ...
    Ev = torch.linalg.solve(torch.diag(normvect),
    ...
    for i in range(1, NCut_MaxTrial+1):
        ...
        EigenVectors = EigenVectors / torch.tile(vm,
                                                 (1, k))  # normalize eigenvectors to ensure each FN vector's norm = 1
    ...
    for ki in range(K):
        if torch.sum(C == ki) > 1:
            ...
            corrW = torch.abs(
                mat_corr_torch(candSet, dataPrecision=dataPrecision))  # Get the similarity between candidate FNs
            corrW[torch.isnan(corrW)] = 0
            mInd = torch.argmax(torch.sum(corrW, dim=0),
                                dim=0)  # Find the FN with the highest total similarity to all other FNs
            gFN[:, ki] = candSet[:, mInd]
        elif torch.sum(C == ki) == 1:
            mInd = int((C == ki).nonzero(as_tuple=True)[0])
            gFN[:, ki] = gFN_BS[:, mInd]

    gFN = gFN / torch.maximum(torch.tile(torch.max(gFN, dim=0)[0], (gFN.shape[0], 1)),
                              torch_eps)  # Normalize each FN by its max value
    print(f'\nFinished at ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '\n', file=logFile,
          flush=True)
...
def gFN_SR_NMF_boostrapping_cluster(dir_pnet_result: str, jobID=1):
    ...
    print('Start to SR-NMF for this bootstrap at ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())),
          flush=True)
    dir_pnet_BS = os.path.join(dir_pnet_FNC, 'BootStrapping')

    # load data
    file_scan_list = os.path.join(dir_pnet_BS, str(jobID), 'Scan_List.txt')
    Data, CHeader, NHeader = load_fmri_scan(file_scan_list, dataType=dataType, dataFormat=dataFormat, nTPoints=nTPoints,
                                            Reshape=True, Brain_Mask=Brain_Mask,
    ...
    # perform NMF
    FN_BS = gFN_SR_NMF_torch(Data, K, gNb, maxIter=maxIter_gFN, init=init, minIter=minIter_gFN, error=error,
                             normW=normW,
...
def fuse_FN_cluster(dir_pnet_result: str):
    ...
    print('Start to fuse bootstrapped results using NCut at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                              time.localtime(time.time())), flush=True)
    dir_pnet_BS = os.path.join(dir_pnet_FNC, 'BootStrapping')
    K = setting['FN_Computation']['K']
    ...
    for rep in range(1, nBS + 1):
        FN_BS[rep - 1] = np.array(
            reshape_FN(load_matlab_single_array(os.path.join(dir_pnet_BS, str(rep), 'FN.mat')), dataType=dataType,
