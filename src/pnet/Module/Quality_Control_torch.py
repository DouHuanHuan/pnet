...
def run_quality_control_torch_cluster(dir_pnet_result: str):
    ...
    while np.sum(flag_complete) < nFolder:
        ...
        for rep in range(1, 1 + nFolder):
            if flag_complete[rep - 1] == 0 and os.path.isfile(
                    os.path.join(dir_pnet_QC, list_subject_folder_unique[rep - 1], 'Result.mat')):
    ...
    for i in range(1, 1 + nFolder):
        ...
        if Result['Miss_Match'][0, 0].shape[0] > 0:
            flag_QC += 1
            print(' ' + str(Result['Miss_Match'][0, 0].shape[0]) + ' miss matched FNs in sub folder: ' +
                  list_subject_folder_unique[i - 1],
                  file=file_Final_Report, flush=True)
    # Finish the final report
    if flag_QC == 0:
