...
def construct_Laplacian_gNb(gNb: np.ndarray, dim_space, vxI=0, X=None, alphaL=10, normW=1, dataPrecision='double'):
    ...
    W = scipy.sparse.coo_matrix((np.ones(gNb.shape[0]), (gNb[:, 0] - 1, gNb[:, 1] - 1)), shape=(dim_space, dim_space),
                                dtype=np_float)
    if vxI > 0:
...
def pFN_NMF(Data, gFN, gNb, maxIter=1000, minIter=30, meanFitRatio=0.1, error=1e-4, normW=1,
    ...
    if Data.shape[1] != gFN.shape[0]:
        raise ValueError(
            "The second dimension of Data should match the first dimension of gFn, as they are space dimension")
...
def gFN_NMF(Data, K, gNb, init='random', maxIter=1000, minIter=200, error=1e-8, normW=1,
            Alpha=2, Beta=30, alphaS=0, alphaL=0, vxI=0, ard=0, eta=0, nRepeat=5, dataPrecision='double',
            logFile='Log_pFN_NMF.log'):
    """
    ...
    for repeat in range(1, 1 + nRepeat):
        ...
        for i in range(1, 1 + maxIter):
            ...
            print(f"    Iter = {i}: LogL: {LogL}, dataFit: {LDf}, spaLap: {LSl}, L21: {L21}, ardU: {ardU}",
                  file=logFile, flush=True)
...
def gFN_fusion_NCut(gFN_BS, K, NCut_MaxTrial=100, dataPrecision='double', logFile='Log_gFN_fusion_NCut'):
    ...
    if isinstance(logFile, str):
        logFile = open(logFile, 'a')
    print(f'\nStart NCut for gFN fusion using NumPy at ' + time.strftime('%Y-%m-%d %H:%M:%S',
    ...
    nW = np.exp(-np.power(nDis, 2) / (
        np.power(np.median(nDisVec), 2)))  # Transform distance values using exp(-X/std^2) with std as the median value
    nW[np.isnan(nW)] = 0
    sumW = np.sum(nW, axis=1)  # total distance for each FN
    sumW[sumW == 0] = 1  # In case two FNs are the same
    # Construct Laplacian matrix
    D = np.diag(sumW)
    L = np.sqrt(np.linalg.inv(D)) @ nW @ np.linalg.inv(
        np.sqrt(D))  # A way to normalize nW based on the total distance of each FN
    L = (L + L.T) / 2  # Ensure L is symmetric. Computation error may result in asymmetry

    eVal, Ev = scipy.sparse.linalg.eigs(L.astype(np.float64), K,
                                        which='LR')  # Get first K eigenvectors, sign of vectors may be different to MATLAB results
    Ev = np.real(Ev)
    ...
    Ev = np.linalg.solve(np.diag(normvect),
                         Ev)  # Use linear solution to normalize Ev satisfying normvect * Ev_new = Ev_old
    ...
    for ki in range(K):
        if np.sum(C == ki) > 1:
            ...
            mInd = np.argmax(np.sum(corrW, axis=0),
                             axis=0)  # Find the FN with the highest total similarity to all other FNs
            gFN[:, ki] = candSet[:, mInd]
        elif np.sum(C == ki) == 1:
            mInd = C.tolist().index(ki)
            gFN[:, ki] = gFN_BS[:, mInd]

    gFN = gFN / np.maximum(np.tile(np.max(gFN, axis=0), (gFN.shape[0], 1)),
                           np_eps)  # Normalize each FN by its max value
    print(f'\nFinished at ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '\n', file=logFile,
          flush=True)
...
def compute_gNb(Brain_Template, logFile=None):
    ...
    if Brain_Template['Data_Type'] == 'Surface':
        ...
        gNb = np.concatenate((gNb_L, gNb_R + len(vL)),
                             axis=0)  # Shift index in right hemisphere by the number of useful vertices in left hemisphere
            ...
            for yi in range(sy):
                for zi in range(sz):
                    if Brain_Mask[xi, yi, zi] > 0:
                        ...
                        patchBox = (np.maximum((xi - 1, yi - 1, zi - 1), (0, 0, 0)),
                                    np.minimum((xi + 2, yi + 2, zi + 2), (sx, sy, sz)))
        ...
        raise ValueError('Unknown combination of Data_Type and Data_Surface: ' + Brain_Template['Data_Type'] + ' : ' +
                         Brain_Template['Data_Format'])

    if len(np.unique(gNb[:, 0])) != max(np.unique(gNb[:, 0])):
        if logFile is not None:
            if isinstance(logFile, str):
                logFile = open(logFile, 'a')
            print('\ngNb contains isolated voxel or vertex which will affect the subsequent analysis', file=logFile,
                  flush=True)
...
def bootstrap_scan(dir_output: str, file_scan: str, file_subject_ID: str, file_subject_folder: str, file_group_ID=None,
                   combineScan=0,
    ...
    if logFile is not None:
        if isinstance(logFile, str):
            logFile = open(logFile, 'a')
        print(f'\nStart preparing bootstrapped scan list files ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                 time.localtime(time.time())) + '\n',
              file=logFile, flush=True)
    ...
    for i in range(1, nBS +1):
        ...
        if samplingMethod == 'Subject':
            ...
            for j in range(sampleSize):
                if combineScan == 1:
                    ...
                    temp = list_scan[
                        np.where(np.compare_chararrays(list_subject_ID, subject_ID_unique[ps[j]], '==', False))[0]]
                    List_BS[j] = str.join('\n', temp)
                else:
        ...
        if logFile is not None:
            print('\nWrite bootstrapped scan list in file ' + os.path.join(dir_output, str(i), 'Scan_List.txt'),
                  file=logFile)
...
def setup_SR_NMF(dir_pnet_result: str, K=17, Combine_Scan=False, file_gFN=None, init='random', samplingMethod='Scan',
                 sampleSize='Automatic', nBS=50, nTPoints=99999, maxIter=(2000, 500), minIter=200, meanFitRatio=0.1,
                 error=1e-8,
                 normW=1, Alpha=2, Beta=30, alphaS=0, alphaL=0, vxI=0, ard=0, eta=0, nRepeat=5, Parallel=False,
                 Computation_Mode='CPU', N_Thread=1, dataPrecision='double', outputFormat='Both'):
    """
    ...
    BootStrap = {'samplingMethod': samplingMethod, 'sampleSize': sampleSize, 'nBS': nBS, 'nTPoints': nTPoints,
                 'init': init}
...
def setup_pFN_folder(dir_pnet_result: str):
    ...
    if combineScan and len(list_subject_folder_unique) == len(list_subject_folder):
        raise ValueError(
            'When combineScan is enabled, the txt file Subject_Folder.txt is supposed to show repeated sub-folder names')
...
def run_FN_Computation(dir_pnet_result: str):
    ...
    print('\nStart FN computation using Numpy at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                   time.localtime(time.time())) + '\n',
    ...
    if setting['FN_Computation']['Method'] == 'SR-NMF':
        print('FN computation uses sparsity-regularized non-negative matrix factorization method', file=logFile_FNC,
              flush=True)
        ...
        if setting['FN_Computation']['Group_FN']['file_gFN'] is None:
            ...
            print(
                'Start to NMF for each bootstrap at ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())),
                file=logFile_FNC, flush=True)
            for rep in range(1, 1 + nBS):
                ...
                Data, CHeader, NHeader = load_fmri_scan(file_scan_list, dataType=dataType, dataFormat=dataFormat,
                                                        nTPoints=nTPoints, Reshape=True, Brain_Mask=Brain_Mask,
                                                        Normalization='vp-vmax', logFile=logFile)
                # perform NMF
                FN_BS = gFN_NMF(Data, K, gNb, init=init, maxIter=maxIter_gFN, minIter=minIter_gFN, error=error,
                                normW=normW,
            ...
            # step 2 ============== fuse results
            # Generate gFNs
            print('Start to fuse bootstrapped results using NCut at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                      time.localtime(time.time())),
                  file=logFile_FNC, flush=True)
            FN_BS = np.empty(nBS, dtype=np.ndarray)
            # load bootstrapped results
            for rep in range(1, nBS + 1):
                FN_BS[rep - 1] = np.array(
                    reshape_FN(load_matlab_single_array(os.path.join(dir_pnet_BS, str(rep), 'FN.mat')),
                               dataType=dataType, Brain_Mask=Brain_Mask))
            ...
            if dataType == 'Volume':
                Brain_Mask = load_brain_template(os.path.join(dir_pnet_dataInput, 'Brain_Template.json.zip'))[
                    'Brain_Mask']
                gFN = reshape_FN(gFN, dataType=dataType, Brain_Mask=Brain_Mask)
        ...
        for i in range(1, N_Scan + 1):
            ...
            # perform NMF
            TC, pFN = pFN_NMF(Data, gFN, gNb, maxIter=maxIter_pFN, minIter=minIter_pFN, meanFitRatio=meanFitRatio,
