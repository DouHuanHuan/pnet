        ...
        if dataFormat == 'HCP Surface (*.cifti, *.mat)':
            if scan_list[i].endswith('.dtseries.nii'):
                ...
                if scan_data.shape[0] < 59412:
                    raise ValueError(
                        'The MATLAB file contains a 2D matrix with the spatial dimension smaller than 59412 in file ' +
                        scan_list[i])
            ...
            if len(str.split(scan_list[i], ';')) != 2:
                raise ValueError(
                    "For MGH surface data format, directories of two hemisphere data need to be combined into one line with ';' as separator")
...
def compute_brain_surface(file_surfL: str,
    ...
    if dataType == 'Surface' and templateFormat == 'HCP':
        ...
        if file_surfL_inflated is not None and file_surfR_inflated is not None:
            ...
            Brain_Surface['Shape_Inflated']['R']['vertices'] = np.round(
                Brain_Surface['Shape_Inflated']['R']['vertices'], n_digit)
        ...
        # Surface shape
        # Index starts from 1
        Brain_Surface['Shape']['L']['vertices'], Brain_Surface['Shape']['L']['faces'] = nib.freesurfer.io.read_geometry(
            file_surfL)
        Brain_Surface['Shape']['R']['vertices'], Brain_Surface['Shape']['R']['faces'] = nib.freesurfer.io.read_geometry(
            file_surfR)
        # Truncating digits to save space
        Brain_Surface['Shape']['L']['vertices'] = np.round(Brain_Surface['Shape']['L']['vertices'], n_digit)
        Brain_Surface['Shape']['R']['vertices'] = np.round(Brain_Surface['Shape']['R']['vertices'], n_digit)
        # Surface shape inflated
        if file_surfL_inflated is not None and file_surfR_inflated is not None:
            Brain_Surface['Shape_Inflated']['L']['vertices'], Brain_Surface['Shape_Inflated']['L'][
                'faces'] = nib.freesurfer.io.read_geometry(file_surfL_inflated)
            Brain_Surface['Shape_Inflated']['R']['vertices'], Brain_Surface['Shape_Inflated']['R'][
                'faces'] = nib.freesurfer.io.read_geometry(file_surfR_inflated)
            # Truncating digits to save space
            Brain_Surface['Shape_Inflated']['L']['vertices'] = np.round(
                Brain_Surface['Shape_Inflated']['L']['vertices'], n_digit)
            Brain_Surface['Shape_Inflated']['R']['vertices'] = np.round(
...
def compute_brain_template(dataType: str,
    ...
    if dataType == 'Volume' and templateFormat == '3D Matrix':
        ...
        Brain_Mask, _, _ = load_fmri_scan(file_mask_vol, dataType=dataType,
                                          dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False,
                                          Normalization=None)
        Overlay_Image, _, _ = load_fmri_scan(file_overlayImage, dataType=dataType,
                                             dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False,
                                             Normalization=None)
        ...
        print_log(f"There are {np.sum(Brain_Template['Brain_Mask']['L'])} vertices in the left hemisphere \nand "
                  f"{np.sum(Brain_Template['Brain_Mask']['R'])} vertices in the right hemisphere", stop=False,
                  logFile=logFile)
        ...
        if isinstance(maskValue, int):
            ...
            print_log(f'Setting mask value = {maskValue[0]} for surface, and mask value = {maskValue[1]} for volume',
                      stop=False, logFile=logFile)
        else:
            print_log(
                'maskValue could be one integer for both surface and volume parts, or a tuple containing two integers',
                stop=True, logFile=logFile)

        # load surface part
        if file_surfL is None or file_surfR is None or file_maskL is None or file_maskR is None:
            raise ValueError(
                'When data type is surface-volume, file_surfL, file_surfR, file_maskL and file_maskR are required')
        Brain_Template = \
            compute_brain_surface(file_surfL, file_surfR, file_maskL, file_maskR,
                                  file_surfL_inflated=file_surfL_inflated, file_surfR_inflated=file_surfR_inflated,
        ...
        Volume_Mask, _, _ = load_fmri_scan(file_mask_vol, dataType='Volume',
                                           dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False,
                                           Normalization=None)
        Overlay_Image, _, _ = load_fmri_scan(file_overlayImage, dataType='Volume',
                                             dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False,
                                             Normalization=None)
        ...
        print_log(f"There are {np.sum(Brain_Template['Surface_Mask']['L'])} vertices in the left hemisphere \nand "
                  f"{np.sum(Brain_Template['Surface_Mask']['R'])} vertices in the right hemisphere", stop=False,
                  logFile=logFile)
        print_log(f"There are {np.sum(Volume_Mask)} voxels in the volume-based part of the brain", stop=False,
                  logFile=logFile)
...
def save_brain_template(dir_pnet_dataInput: str,
    ...
    if Brain_Template['Data_Type'] == 'Volume':
        ...
        if 'Shape_Inflated' in Brain_Template.keys():
            # only reserve 2 digits
            Brain_Template['Shape_Inflated']['L']['vertices'] = list(
                map(partial, Brain_Template['Shape_Inflated']['L']['vertices'].tolist()))
            Brain_Template['Shape_Inflated']['R']['vertices'] = list(
                map(partial, Brain_Template['Shape_Inflated']['R']['vertices'].tolist()))
...
def load_brain_template(file_Brain_Template: str,
    ...
    if Brain_Template['Data_Type'] == 'Volume':
        ...
        if 'vertices' not in Brain_Template['Shape']['L'].keys() or 'vertices' not in Brain_Template['Shape'][
            'L'].keys():
        ...
        if 'vertices' not in Brain_Template['Shape']['R'].keys() or 'vertices' not in Brain_Template['Shape'][
            'R'].keys():
        ...
        if 'Shape_Inflated' in Brain_Template.keys():
            # check sub-keys
            if 'vertices' not in Brain_Template['Shape_Inflated']['L'].keys() or 'vertices' not in \
                    Brain_Template['Shape_Inflated']['L'].keys():
                raise ValueError("Cannot find vertices in Brain_Template['Shape_Inflated']['L']")
            if 'faces' not in Brain_Template['Shape_Inflated']['L'].keys() or 'faces' not in \
                    Brain_Template['Shape_Inflated']['L'].keys():
                raise ValueError("Cannot find faces in Brain_Template['Shape_Inflated']['L']")
            if 'vertices' not in Brain_Template['Shape_Inflated']['R'].keys() or 'vertices' not in \
                    Brain_Template['Shape_Inflated']['R'].keys():
                raise ValueError("Cannot find vertices in Brain_Template['Shape_Inflated']['R']")
            if 'faces' not in Brain_Template['Shape_Inflated']['R'].keys() or 'faces' not in \
                    Brain_Template['Shape_Inflated']['R'].keys():
                raise ValueError("Cannot find faces in Brain_Template['Shape_Inflated']['R']")
            # convert to np.ndarray
            Brain_Template['Shape_Inflated']['L']['vertices'] = np.array(
                Brain_Template['Shape_Inflated']['L']['vertices'])
            Brain_Template['Shape_Inflated']['R']['vertices'] = np.array(
                Brain_Template['Shape_Inflated']['R']['vertices'])
...
def setup_cifti_volume(file_cifti: str,
    ...
    print_log('Created a NIFTI file for CIFTI volume part, the mask value represents the index order', stop=False,
              logFile=logFile)
    ...
    if dataType == 'Volume':
        if len(scan_data.shape) == 4:  # 4D fMRI data, reshape to 2D [dim_time, dim_space]
            if scan_data.shape[0:3] != Brain_Mask.shape:
                raise ValueError(
                    'The shapes of Brain_Mask and scan_data are not the same when scan_data is a 4D matrix')
            reshaped_data = np.reshape(scan_data, (np.prod(scan_data.shape[0:3]), scan_data.shape[3]),
                                       order='F').T  # Match colum based index used in MATLAB
...
def reshape_FN(FN: np.ndarray,
    ...
    if dataType == 'Volume':
        if len(FN.shape) == 4:  # 4D FN [X Y Z K], reshape to 2D [dim_space, K]
            ...
            reshaped_FN = np.reshape(FN, (np.prod(FN.shape[0:3]), FN.shape[3]),
                                     order='F')  # Match colum based index used in MATLAB
            ...
            if Volume_Order is not None:
                raise ValueError('need update')
            reshaped_FN = np.reshape(reshaped_FN,
                                     (Brain_Mask.shape[0], Brain_Mask.shape[1], Brain_Mask.shape[2], FN.shape[1]),
                                     order='F')  # Match colum based index used in MATLAB
            ...
            reshaped_FN = np.reshape(reshaped_FN, (Brain_Mask.shape[0], Brain_Mask.shape[1], Brain_Mask.shape[2]),
                                     order='F')  # Match colum based index used in MATLAB
...
def check_data_type_format(dataType: str,
    ...
    if dataType == 'Volume' and dataFormat not in ('Volume (*.nii, *.nii.gz, *.mat)', 'HCP Volume (*.cifti)'):
        print_log(
            "When dataType is volume, dataFormat should be 'Volume (*.nii, *.nii.gz, *.mat)' or 'HCP Volume (*.cifti)'",
            logFile=logFile, stop=True)
...
def setup_scan_info(dir_pnet_dataInput: str,
    ...
    if scan_info == 'Manual':
        ...
        if file_subject_ID is None and file_subject_folder is None:
            ...
            list_subject_ID = [os.path.normpath(list_scan[i][len(root_directory) + 1: -1]).split(os.path.sep)[0] for i
                               in range(N_Scan)]
...
def output_FN(FN: np.ndarray or str or tuple,
    ...
    # Check input
    if isinstance(FN,
                  np.ndarray) and file_output is None:  # isinstance(file_output, str):  modified by FY on 07/26/2024
        print_log("file_output needs to be a non-empty directory, when input FN is an np.ndarray", stop=True,
                  logFile=logFile)
    elif (isinstance(FN, str) or isinstance(FN, tuple)) and file_output is not None:
        print_log("file_output needs to be None, when input FN is a string or a tuple of string", stop=True,
                  logFile=logFile)
