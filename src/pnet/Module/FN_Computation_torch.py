...
def run_FN_Computation_torch(dir_pnet_result: str):
    ...
    print('\nStart FN computation using PyTorch at ' + time.strftime('%Y-%m-%d %H:%M:%S',
    ...
    if FN_Method == 'SR-NMF':
        ...
        if setting['FN_Computation']['Group_FN']['file_gFN'] is None:
            ...
            for rep in range(1, 1 + nBS):
                ...
                # perform NMF
                FN_BS = SR_NMF.gFN_SR_NMF_torch(Data, K, gNb, init=init, maxIter=maxIter_gFN, minIter=minIter_gFN,
                                                error=error, normW=normW,
                                                Alpha=Alpha, Beta=Beta, alphaS=alphaS, alphaL=alphaL, vxI=vxI, ard=ard,
                                                eta=eta,
        ...
        for i in range(1, N_Scan + 1):
            ...
            # perform NMF
            TC, pFN = SR_NMF.pFN_SR_NMF_torch(Data, gFN, gNb, maxIter=maxIter_pFN, minIter=minIter_pFN,
        ...
        gFN, _, _ = load_fmri_scan(file_gFN, dataType='Volume', dataFormat='Volume (*.nii, *.nii.gz, *.mat)',
                                   Reshape=False, Normalization=None)
        if dataType == 'Volume':
    ...
    if FN_Method == 'SR-NMF':
        ...
        if setting['FN_Computation']['Group_FN']['file_gFN'] is None:
            ...
            for i in range(1, 1 + nBS):
                if os.path.exists(os.path.join(dir_pnet_BS, str(i))) and os.path.isfile(
                        os.path.join(dir_pnet_BS, str(i), 'Scan_List.txt')):
                    flag_complete[i - 1] = 1
            ...
            while np.sum(flag_complete) < nBS:
                ...
                if Count % report_interval == 0:
                    print(f'--> Found {np.sum(flag_complete)} finished jobs out of {nBS} at ' + time.strftime(
            ...
            while flag_complete == 0:
                ...
                Count += 1
                if Count % report_interval == 0:
                    print(f'--> Found {np.sum(flag_complete)} finished jobs out of 1 at ' + time.strftime(
                        '%Y-%m-%d %H:%M:%S', time.localtime(time.time())), flush=True)
        ...
        file_gFN = setting['FN_Computation']['Group_FN']['file_gFN']
        # gFN = load_matlab_single_array(file_gFN) # have to be 'Volume'
        gFN = load_fmri_scan(file_gFN, dataType='Volume', dataFormat='Volume (*.nii, *.nii.gz, *.mat)', Reshape=False,
                             Normalization=None)
        if dataType == 'Volume':
            Brain_Mask = load_brain_template(os.path.join(dir_pnet_dataInput, 'Brain_Template.json.zip'))['Brain_Mask']
            gFN = reshape_FN(gFN, dataType=dataType, Brain_Mask=Brain_Mask)
