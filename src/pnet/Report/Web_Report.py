...
def run_web_report(dir_pnet_result: str):
    ...
    print_log('\nStart generating HTML based web report at ' + time.strftime('%Y-%m-%d %H:%M:%S',
    ...
    for i in range(nFolder):
        if os.path.isfile(os.path.join(dir_pnet_pFN, list_subject_folder_unique[i], 'All.jpg')) and not os.path.isfile(
                os.path.join(dir_pnet_pFN, list_subject_folder_unique[i], 'All(Compressed).jpg')):
    ...
    if setting['FN_Computation']['Group_FN']['file_gFN'] is None:
        ...
        text_gFN = 'The group FNs are loaded from precomputed results at ' + setting['FN_Computation']['Group_FN'][
            'file_gFN']
    ...
    # pFN examples
    nMax = 10
    frames = [Image.open(os.path.join(dir_pnet_pFN, list_subject_folder[subject_index[i]], 'All(Compressed).jpg')) for i
              in range(np.minimum(nMax, nSubject))]
    frame_one = frames[0]
    frame_one.save(os.path.join(dir_pnet_pFN, 'Example.gif'), format="GIF", append_images=frames, save_all=True,
    ...
    if n_missmatch == 0:
        ...
        text_qc = 'There are ' + str(n_pass) + ' out of ' + str(
            n_pass + n_missmatch) + ' ' + scan_subject + ' passed QC. <br />\n' + \
                  'There are ' + str(
            n_missmatch) + ' scans do not pass QC, meaning that they have at least one pFN showing smaller similarity to their group-level counterpart. <br />\n'
    ...
    for i in range(nFolder):
        if list_subject_ID[folder_index[i]] != pre_sub:
            ...
            with open(template_individual, 'r') as file:
                html_as_string = file.read()
            # copy gFN figure
            shutil.copyfile(os.path.join(dir_pnet_gFN, 'All(Compressed).jpg'),
                            os.path.join(dir_pnet_pFN, list_subject_folder_unique[i], 'gFN_All(Compressed).jpg'))
            # report title
            ...
            if np.sum(delta_SC[i, :] < 0) == 0:
                ...
                Result = load_matlab_single_variable(
                    os.path.join(dir_pnet_QC, list_subject_folder_unique[i], 'Result.mat'))
