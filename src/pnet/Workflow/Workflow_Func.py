...
def workflow(dir_pnet_result: str,
    ...
    dir_pnet_dataInput, dir_pnet_FNC, dir_pnet_gFN, dir_pnet_pFN, dir_pnet_QC, dir_pnet_STAT = setup_result_folder(
        dir_pnet_result)
...
def workflow_guide():
    ...
    Choice = guide_YN("Do you have a txt formatted file containing subject ID information for each corresponding scan?",
                      skip=True, default_value='N')
    if Choice == 'Y':
        file_subject_ID = guide_file(
            "Provide a txt formatted file containing subject ID information for each corresponding scan:", existed=True,
            extension='.txt')
        Choice = guide_YN(
            "Do you have a txt formatted file containing subject folder information for each corresponding scan?")
        if Choice == 'Y':
            file_subject_folder = guide_file(
                "Provide a txt formatted file containing subject folder information for each corresponding scan:",
                existed=True, extension='.txt')
        else:
    ...
    if Choice == 'Y':
        file_Brain_Template = guide_choice("Select a built-in brain template:", (
            'HCP Surface', 'MNI Volume', 'FreeSurfer_fsaverage5', 'HCP Surface-Volume', 'HCP Subcortical Volume'))
        ...
        if Choice == 'Y':
            file_Brain_Template = guide_file("Set up the directory of the brain template file in json format:",
                                             existed=True, extension='.json')
        else:
            file_Brain_Template = None
            # Volume and surface data types require different inputs to compute the brain template
            if dataType == 'Volume':
                templateFormat = guide_choice("Choose a template format:", ('3D Matrix', 'HCP'))
                file_mask_vol = guide_file("Set up the directory of a brain mask:", existed=True,
                                           extension=('.mat', '.nii', '.nii.gz'))
                file_overlayImage = guide_file(
                    "Set up the directory of a high resolution T1/T2 image as the overlay background:", existed=True,
                    extension=('.mat', '.nii', '.nii.gz'))
                maskValue = guide_number("What is the value used for labeling useful voxels in the brain mask file?",
                                         'Int')
            elif dataType == 'Surface':
                templateFormat = guide_choice("Choose a template format:", ('HCP', 'FreeSurfer'))
                file_surfL = guide_file(
                    "Set up the directory of the left hemisphere brain shape file (ex. Conte69.L.inflated.32k_fs_LR.surf.gii):",
                    existed=True, extension='.surf.gii')
                file_surfR = guide_file(
                    "Set up the directory of the left hemisphere brain shape file (ex. Conte69.R.inflated.32k_fs_LR.surf.gii):",
                    existed=True, extension='.surf.gii')
                Choice = guide_YN("Would you like to load an inflated brain surface shape?", skip=True,
                                  default_value='N')
                if Choice == 'Y':
                    file_surfL = guide_file(
                        "Set up the directory of the left hemisphere brain shape file (ex. Conte69.L.very_inflated.32k_fs_LR.surf.gii):",
                        existed=True, extension='.surf.gii')
                    file_surfR = guide_file(
                        "Set up the directory of the left hemisphere brain shape file (ex. Conte69.R.very_inflated.32k_fs_LR.surf.gii):",
                        existed=True, extension='.surf.gii')
                else:
                    file_surfL_inflated = None
                    file_surfR_inflated = None
                file_maskL = guide_file(
                    "Set up the directory of the left hemisphere brain mask file (ex. medial_wall.L.32k_fs_LR.func.gii):",
                    existed=True, extension='.surf.gii')
                file_maskR = guide_file(
                    "Set up the directory of the left hemisphere brain mask file (ex. medial_wall.R.32k_fs_LR.func.gii):",
                    existed=True, extension='.surf.gii')
                maskValue = guide_number("Set up the value used for labeling useful voxels in the brain mask file?",
                                         'Int')
            elif dataType == 'Surface-Volume':
                templateFormat = guide_choice("Choose a template format:", ('HCP', 'FreeSurfer'))
                file_surfL = guide_file(
    ...
    if method == 'SR-NMF':
        Choice = guide_YN("Do you want to load a precomputed group-level functional networks?", skip=True,
                          default_value='N')
    else:
        Choice = 'Y'
    if Choice == 'Y':
        file_gFN = guide_file(
            'Set up the file directory of the precomputed group-level functional networks in matlab format?',
            existed=True, extension='.mat')
    else:
        file_gFN = None
    ...
    print('# ============================ #')
    file_script = guide_file("Setup a Python file directory (*.py) of this customized workflow:", existed=False,
                             extension='.py')

    file_script = open(file_script, 'w')
    print('# Customized Python script for pNet workflow, built at ' + time.strftime('%Y-%m-%d %H:%M:%S',
...
def workflow_cluster(dir_pnet_result: str,
    ...
    print(
        '# Customized Python script for pNet workflow in cluster computation\n# Use corresponding bash script to submit the job\n# Created on ' + time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(time.time())), file=file_script)
    ...
    file_pnet_workflow_cluster_template = os.path.join(dir_pnet, 'Workflow',
                                                       'Workflow_Cluster_Template.py')  # 'Python', 'Workflow', 'Workflow_Cluster_Template.py')
