...
def colorize(value_map: np.ndarray, color_function: np.ndarray):
    ...
    if color_function.shape[1] != 4 or color_function.shape[0] < 2 or not np.all(
            color_function[:, 0] == np.sort(color_function[:, 0])):
...
def assemble_image(file_list_image: tuple, file_output_assembled=None or str, organization=(0, 10), interval=(50, 5),
                   background=(0, 0, 0)):
    """
    ...
    for x in range(organization[0]):
        for y in range(organization[1]):
            ...
            if x == 0 and y == 0:
                image_assembled = np.zeros((image_sub.shape[0] * organization[0] + (organization[0] - 1) * interval[0],
                                            image_sub.shape[1] * organization[1] + (organization[1] - 1) * interval[1],
                                            3), dtype=np.uint8)
            ...
            if x < organization[0] - 1:
                image_assembled[ps[0] + image_sub.shape[0]:ps[0] + image_sub.shape[0] + interval[0],
                ps[1]:ps[1] + image_sub.shape[1], :] = \
                    np.reshape(background, (1, 1, 3))
            if y < organization[1] - 1:
                image_assembled[ps[0]:ps[0] + image_sub.shape[0],
                ps[1] + image_sub.shape[1]:ps[1] + image_sub.shape[1] + interval[1], :] = \
                    np.reshape(background, (1, 1, 3))
            if x < organization[0] - 1 and y < organization[1] - 1:
                image_assembled[ps[0]:ps[0] + image_sub.shape[0] + interval[0],
                ps[1] + image_sub.shape[1]:ps[1] + image_sub.shape[1] + interval[1], :] = \
                    np.reshape(background, (1, 1, 3))
...
def plot_brain_surface(brain_map: np.ndarray,
    ...
    p = surfplot.Plot(surf_lh=polyData, zoom=view_angle, views=orientation, background=background, brightness=1,
                      size=figure_size)

    # brain surface and mask layer
    map_mask = (mask == 0).astype(np.float32)
    color_function_brain = np.array(
        ((0, brain_color[0], brain_color[1], brain_color[2]), (1, mask_color[0], mask_color[1], mask_color[2])),
        dtype=np.float32)
    ...
    p.add_layer(map_2, cmap=prepare_color_map(color_function=color_function), color_range=color_range, cbar=None,
...
def merge_mesh_LR(mesh_LR: dict, offset=np.array((90, 0, 0))):
    ...
    mesh = {'vertices': np.concatenate((mesh_LR['L']['vertices'], mesh_LR['R']['vertices'] + offset), axis=0),
            'faces': np.concatenate((mesh_LR['L']['faces'], mesh_LR['R']['faces'] + mesh_LR['L']['vertices'].shape[0]),
                                    axis=0)}
    return mesh
...
def plot_FN_brain_surface_5view(brain_map: np.ndarray,
    ...
    # dorsal view
    image_rgb = plot_brain_surface(brain_map, mesh=merge_mesh_LR(brain_template['Shape'],
                                                                 offset=np.array((hemisphere_offset, 0, 0))),
                                   mask=merge_mask_LR(brain_template['Brain_Mask']),
    ...
    axs[1].axis('off')
    axs[1].set_title(label=figure_title, loc='center', pad=140, fontsize=150, fontweight='bold', fontname='Arial',
                     color=(1, 1, 1))

    # saggital views
    # 1st
    image_rgb = plot_brain_surface(brain_map[0:Nv_L], mesh=brain_template['Shape']['L'],
                                   mask=brain_template['Brain_Mask']['L'],
                                   color_function=color_function,
    ...
    axs[3].axis('off')
    # 3rd
    image_rgb = plot_brain_surface(brain_map[Nv_L:], mesh=brain_template['Shape']['R'],
                                   mask=brain_template['Brain_Mask']['R'],
                                   color_function=color_function,
                                   orientation='medial', view_angle=view_angle[1], file_output=None, dpi=dpi)
    ...
    X = np.tile(np.arange(color_range[0], color_range[1], (color_range[1] - color_range[0]) / colorbar_step),
                (colorbar_ratio, 1))
...
def plot_voxel_map_3view(Anatomy: np.ndarray, Voxel_Map: np.ndarray, center: np.ndarray, color_function: np.ndarray,
    ...
    if organization in ([[1, 3], [2, 0]], [[2, 3], [1, 0]]):  # MATLAB index
        ...
        image_rgb = np.zeros(
            (Voxel_Map2D[organization[0, 0] - 1].shape[0] + Voxel_Map2D[organization[1, 0] - 1].shape[0] + interval,
             Voxel_Map2D[organization[0, 0] - 1].shape[1] + Voxel_Map2D[organization[0, 1] - 1].shape[1] + interval,
             3)) + background

        ps_list = (Voxel_Map2D[organization[0, 0] - 1].shape[0], Voxel_Map2D[organization[0, 0] - 1].shape[1],
                   Voxel_Map2D[organization[0, 1] - 1].shape[1],
        ...
        for i, val in enumerate(organization.flatten()):
            image_rgb[i * Dimension[0]:(i + 1) * Dimension[0], :, :] = Anatomy2D[val - 1] * Mask[val - 1] + Voxel_Map2D[
                val - 1] * (1 - Mask[val - 1])
...
def plot_FN_brain_volume_3view(brain_map: np.ndarray,
    ...
    Brain_Mask_2, _, Crop_Parameter = fTruncate_Image_3D_4D(Brain_Mask, Voxel_Size=np.array((1, 1, 1)),
                                                            Extend=np.array((np.inf, np.inf, np.inf)))
    ...
    Crop_Parameter['FOV'] = np.array([[1, s] for s in Map_2.shape]) + np.tile(
        np.round((np.array([Max_Dim] * 3) - np.array(Map_2.shape)) / 2), (2, 1)).T
    ...
    image_rgb = plot_voxel_map_3view(Overlay_Image_2, Map_2, center=Center, rotation=rotation,
                                     organization=organization, color_function=color_function)
    ...
    axs[1].imshow(image_rgb)
    axs[1].axis('off')
    axs[1].set_title(label=figure_title, loc='center', pad=30, fontsize=150, fontweight='bold', fontname='Arial',
                     color=(1, 1, 1))
...
def plot_FN_brain_surface_volume_7view(brain_map: np.ndarray,
    ...
    # 1st
    image_rgb = plot_brain_surface(brain_map[0:Nv_L], mesh=brain_template['Shape']['L'],
                                   mask=brain_template['Surface_Mask']['L'],
                                   color_function=color_function,
    ...
    axs[1].imshow(image_rgb)
    axs[1].axis('off')
    axs[1].set_title(label=figure_title, loc='center', pad=40, fontsize=150, fontweight='bold', fontname='Arial',
                     color=(1, 1, 1))
    ...
    axs[2].axis('off')
    # 3rd
    image_rgb = plot_brain_surface(brain_map[Nv_L:Nv_LR], mesh=brain_template['Shape']['R'],
                                   mask=brain_template['Surface_Mask']['R'],
                                   color_function=color_function,
                                   orientation='medial', view_angle=view_angle, file_output=None, dpi=dpi)
    ...
    # volume view
    brain_map = reshape_FN(brain_map[Nv_LR:], dataType='Volume', Brain_Mask=brain_template['Volume_Mask'],
                           Volume_Order=brain_template['Volume_Order'])
    # upsampling
    upsampling = np.round(brain_template['Overlay_Image'].shape[0] / brain_map.shape[0])
    if np.sum(np.abs(np.array(brain_map.shape) * upsampling - np.array(brain_template['Overlay_Image'].shape))) > 0:
        raise ValueError('the Overlay_Image does NOT have an integer upsampling scale to the brain map')
    ...
    image_rgb = plot_voxel_map_3view(Overlay_Image_2, Map_2, center=Center, rotation=rotation,
                                     organization=organization, color_function=color_function, interval=10)
    volume_width = 0.9
...
def run_gFN_Visualization(dir_pnet_result: str):
    ...
    :return: Dict containing output image paths and metadata

    Yuncong Ma, 2/8/2024 (Modified: ChatGPT, 2025/4)
    """
    import os
    ...
    if dataType == 'Surface' and dataFormat == 'HCP Surface (*.cifti, *.mat)':
        ...
        for i in range(K):
            plot_FN_brain_surface_5view(
                gFN[:, i], brain_template, threshold=threshold, figure_title=f'FN {i + 1}',
                file_output=file_output[i], color_function=None,
                colorbar_range_style=colorbar_range_style,
        ...
        for i in range(K):
            plot_FN_brain_surface_5view(
                gFN[:, i], brain_template, threshold=threshold, figure_title=f'FN {i + 1}',
                file_output=file_output[i], color_function=None, hemisphere_offset=100,
                colorbar_range_style=colorbar_range_style,
                colorbar_scale=colorbar_scale,
                colorbar_range_round=colorbar_range_round,
                colorbar_label=colorbar_label
        ...
        for i in range(K):
            ...
            plot_FN_brain_volume_3view(
                gFN[:, :, :, i], brain_template, threshold=threshold, figure_title=f'FN {i + 1}',
                file_output=file_output[i], file_setting=file_setting, color_function=None,
                colorbar_range_style=colorbar_range_style,
                colorbar_scale=colorbar_scale,
                colorbar_range_round=colorbar_range_round,
                colorbar_label=colorbar_label
    ...
    # output assembled and compressed image
    assembled_path = os.path.join(dir_pnet_gFN, 'All.jpg')
    compressed_path = os.path.join(dir_pnet_gFN, 'All(Compressed).jpg')

    assemble_image(file_output, assembled_path, interval=(50, 5), background=(0, 0, 0))
    compress_image(compressed_path, assembled_path, image_size=(2000, 10000))

    return {
        "FN_method": FN_Method,
        "data_type": dataType,
        "data_format": dataFormat,
        "n_FN": len(file_output),
        "individual_FN_images": file_output,
        "assembled_image": assembled_path,
        "compressed_image": compressed_path
    }


def run_pFN_Visualization(dir_pnet_result: str) -> dict:
    """
    Run preconfigured visualizations for pFNs and return detailed image metadata per subject.

    :param dir_pnet_result: directory of the pnet result folder
    :return: A dict of subject_id -> metadata dictionary
    """
    ...
    setting = {
        'Data_Input': settingDataInput,
        'FN_Computation': settingFNC,
        'Visualization': settingVisualization
    }
    ...
    for scan in range(1, N_Scan + 1):
        ...
        if dataType == 'Surface' and dataFormat == 'HCP Surface (*.cifti, *.mat)':
            ...
            for i in range(K):
                plot_FN_brain_surface_5view(
                    pFN[:, i], brain_template, None, threshold,
            ...
            for i in range(K):
                ...
                plot_FN_brain_volume_3view(
                    ...
                    figure_title=f'FN {i + 1}', file_output=file_output[i],
                    colorbar_range_style=colorbar_range_style, colorbar_scale=colorbar_scale,
                    colorbar_range_round=colorbar_range_round, colorbar_label=colorbar_label
                )
        ...
        # Assemble & compress
        assembled_path = os.path.join(dir_pnet_pFN_indv, 'All.jpg')
        compressed_path = os.path.join(dir_pnet_pFN_indv, 'All(Compressed).jpg')
...
def run_Visualization_cluster(dir_pnet_result: str):
    ...
    while flag_complete == 0:
        ...
        if Count % report_interval == 0:
            print(f'--> Found {np.sum(flag_complete)} finished jobs out of 1 at ' + time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                                  time.localtime(
                                                                                                      time.time())),
                  flush=True)
    ...
    while np.sum(flag_complete) < nFolder:
        ...
        Count += 1
        if Count % report_interval == 0:
            print(f'--> Found {np.sum(flag_complete)} finished jobs out of {nFolder} at ' + time.strftime(
